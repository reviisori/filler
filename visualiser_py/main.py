...
###button variables:
start_button = button(resolution / 2 - 105, resolution / 2 + 125, start_img)
start_button_d = button(resolution / 2 - 105, resolution / 2 + 125, start_d_img)
...
def load_map_resources(sqr_side):
    ...
    o_new = pygame.image.load("./resources/sqr_o_new.png").convert_alpha()
    o_new = pygame.transform.scale(o_new, (sqr_side, sqr_side))
    x_img = pygame.image.load("./resources/sqr_x.png").convert_alpha()
    x_img = pygame.transform.scale(x_img, (sqr_side, sqr_side))
    x_new = pygame.image.load("./resources/sqr_x_new.png").convert_alpha()
    x_new = pygame.transform.scale(x_new, (sqr_side, sqr_side))
    return empty_img, o_img, o_new, x_img, x_new
...
###function to quickly skip x ammount of lines in stdin:
...
def play():
    ...
    while is_playing:
        ...
        empty_img, o_img, o_new, x_img, x_new = load_map_resources(sqr_side)
        x_reset = x_ofs
        y_reset = y_ofs
        ...
        while x < map_x:
            ...
            while y < map_y + 4:
                if s[y] == 'O':
                    ...
                    window.blit(o_new, (y_ofs, x_ofs))
                elif s[y] == 'X':
                    window.blit(x_img, (y_ofs, x_ofs))
                elif s[y] == 'x':
                    window.blit(x_new, (y_ofs, x_ofs))
                else:
        ...
        ##one of the players stopped playing:
        rip = s.rstrip(":\n").split(" ")
        if "Piece" in rip[0]:
...
while is_running:
    ...
    window.blit(logo_img, ((resolution - 600) / 2, resolution / 2 - 60))
    ...
    if not start:
        ...
        window.blit(bye, (bye_y, resolution / 2 + 130))
